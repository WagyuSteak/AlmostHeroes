...
public class GridManager : MonoBehaviour
{
    ...
    {
        return enemyPositions.ContainsKey(position) ? enemyPositions[position].GetComponent<EnemyBase>() : null;
    }
    ...
    public void DeactivateCell(Vector2Int gridPosition)
    {
        if (activatedCellsByCharacters.ContainsKey(gridPosition))
        {
            activatedCellsByCharacters[gridPosition] = false; // 활성화 상태 해제

            // 셀 색상 초기화 또는 다른 시각적 효과 복구
            if (gridCells.ContainsKey(gridPosition))
            {
                Renderer cellRenderer = gridCells[gridPosition].GetComponent<Renderer>();
                if (cellRenderer != null)
                {
                    cellRenderer.material.color = defaultColor; // 기본 색상으로 초기화 (예: 하얀색)
                }
            }
        }
    }
